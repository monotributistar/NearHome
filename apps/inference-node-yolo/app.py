...
NODE_RESOURCES = {
    "cpu": max(0, int(os.environ.get("NODE_RESOURCES_CPU", "8"))),
    "gpu": max(0, int(os.environ.get("NODE_RESOURCES_GPU", "0"))),
    "vramMb": max(0, int(os.environ.get("NODE_RESOURCES_VRAM_MB", "0"))),
}
NODE_TASK_TYPES = [x.strip() for x in os.environ.get("NODE_TASK_TYPES", "object_detection").split(",") if x.strip()]
NODE_MODELS = [x.strip() for x in os.environ.get("NODE_MODELS", "yolo26n@1.0.0,yolo26s@1.0.0").split(",") if x.strip()]
...
app = FastAPI(title="NearHome Inference Node YOLO", version="0.2.0")
...
def infer(payload: InferRequest):
    ...
    return {
        ...
        "providerMeta": {"engine": "yolo", "node": NODE_ID},
    }
