...
export type EdgeGatewaysPluginOptions = {
  middleware: AppMiddleware;
  balenaApiUrl?: string | null;
  balenaApiKey?: string | null;
};
...
export const edgeGatewaysPlugin: FastifyPluginAsync<EdgeGatewaysPluginOptions> = async (app, opts) => {
  ...
  const encryptCameraCredential = (value: string) => {
    const secret = process.env.CAMERA_CREDENTIALS_KEY ?? process.env.JWT_SECRET;
    if (!secret) throw app.httpErrors.serviceUnavailable("Camera credential encryption is not configured");
  ...
  app.post("/api/v1/edge-gateways/register", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (existing)
      return reply
        .status(409)
        .send({ error: "DEVICE_ALREADY_REGISTERED", message: "Device with this UUID is already registered" });
  ...
  app.get("/api/v1/edge-gateways/:id", { preHandler: tenantScopedPreHandler }, async (request: FastifyRequest) => {
    ...
    const edgeGateway = await prisma.edgeGateway.findFirst({
      ...
      include: { discoveredCameras: { orderBy: { lastSeenAt: "desc" } } }
    });
  ...
  app.post(
    "/api/v1/edge-gateways/:id/heartbeat",
    { preHandler: deviceAuthPreHandler },
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { id } = z.object({ id: z.string() }).parse(request.params);
      const edgeGateway = (request as any).edgeGateway;
        ...
        .object({
          ...
          customMetrics: z
            .object({
              ...
              tunnelStatus: z
                .object({ activeTunnels: z.number(), failedTunnels: z.number(), lastFailure: z.string().optional() })
                .optional(),
  ...
  app.post(
    "/api/v1/edge-gateways/:id/cameras/discover",
    { preHandler: deviceAuthPreHandler },
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { id } = z.object({ id: z.string() }).parse(request.params);
      const edgeGateway = (request as any).edgeGateway;
        ...
        .object({
          cameras: z.array(
            z.object({
              ...
              onvifInfo: z.object({ manufacturer: z.string(), model: z.string(), firmware: z.string() }).optional(),
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const discoveredCamera = await prisma.discoveredCamera.findFirst({
        where: { macAddress: body.macAddress, edgeGatewayId: id, tenantId: ctx.tenantId }
      });
      if (!discoveredCamera)
        return reply
          .status(404)
          .send({ error: "CAMERA_NOT_FOUND", message: "Discovered camera not found for this gateway" });
      ...
      const updated = await prisma.discoveredCamera.update({
        ...
        data: {
          ...
          passwordEncrypted: body.password ? encryptCameraCredential(body.password) : null,
          status: "registered",
          tunnelPort: 8554 + Math.floor(Math.random() * 1000)
        }
      });
  ...
  app.get(
    ...
    async (request: FastifyRequest) => {
      ...
      const cameras = await prisma.discoveredCamera.findMany({
        where: { edgeGatewayId: id, tenantId: ctx.tenantId, ...(status ? { status } : {}) },
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (existing) {
        return reply
          .status(409)
          .send({ error: "DUPLICATE_FLEET", message: "Fleet with this name already exists for this tenant" });
      }
  ...
  app.get("/api/v1/fleets", { preHandler: tenantScopedPreHandler }, async (request: FastifyRequest) => {
    const ctx = getTenantContext(request);
  ...
  app.get("/api/v1/fleets/:id", { preHandler: tenantScopedPreHandler }, async (request: FastifyRequest) => {
    const ctx = getTenantContext(request);
    const { id } = z.object({ id: z.string() }).parse(request.params);
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!gateway) {
        return reply
          .status(400)
          .send({ error: "INVALID_GATEWAY", message: "Gateway not found or belongs to different tenant" });
      }
